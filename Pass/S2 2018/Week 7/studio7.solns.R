...
# -------------------------------------------------------------------------------------------
# 2.9
nb = naive_bayes(Disease ~ ., data = gene.train)
pred = predict(nb,gene.test)
prob = predict(nb,gene.test,type="prob")
head(prob) # two columns -- first is prob of failure, second is prob of success
prob = predict(nb,gene.test,type="prob")[,2]   # gets just the prob. of success
my.pred.stats(prob, gene.test$Disease)
...
# -------------------------------------------------------------------------------------------
# 3.9
nb = naive_bayes(DIABETES ~ ., data=pima.train)
prob = predict(nb,pima.test,type="prob")[,2]   # gets just the prob. of success
my.pred.stats(prob, pima.test$DIABETES)

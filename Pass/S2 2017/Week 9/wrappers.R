...
{
  # Compute CV scores
  cvstats <- cv.tree(object=base_tree, FUN=prune.tree, K=nfolds)
  for(i in 2:m)
  {
    cvstats$dev <- cvstats$dev + cv.tree(object = base_tree, FUN=prune.tree, K = nfolds)$dev
  }
  cvstats$dev <- cvstats$dev/m
  
  # Prune tree using the best 'cv' model
  k = cvstats$k[which.min(cvstats$dev)]
  tree.cv = prune.tree(base_tree, k = k * (1-1e-5))
  
  return(list("best.tree"=tree.cv,"cvstats"=cvstats,"best.k"=k))
}
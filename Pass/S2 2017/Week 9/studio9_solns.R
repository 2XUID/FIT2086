...
# The tree has used the variables BMI, S5, S1, S3, BP and S6
# This means that somewhere in the tree there is a split using these each of
# these variables
#
# There are 12 leaf (terminal) nodes in this tree.
# 

# 2.3
tree.diabetes

# To interpret this output, note that each line represents a node in the
#
...
# inviduals. Node 4 is reached by having first a BMI < 27.75, and then an S5 < 4.61005,
...
#
# (-infinity < S5 < 4.16), (4.16 < S5 < 4.61), (4.61 < S5 < 4.88), (4.88 < S5 < infinity)
# 
...
#
# For categorical predictors, several categories may lead to the same leaf when splitting --
...
# then the right-hand branch, then the left-hand branch, then finally the left-hand branch
...
# 2.6
cv = learn.tree.cv(tree.diabetes,nfolds=10,m=1000)
plot(cv$cv)
#
# The CV plot shows that the best tree size is around 8. This is the number
...
#
# As CV is a random process, sometimes it will produce a slightly different
# estimate of the number of leaf nodes to use, but they will generally be around 
# 8. Taking m=1000 reduces the chance of a different size tree than 8 having the
...
prune.tree(tree.diabetes, k = cv$best.k)
# This prunes the tree using the optimal pruning parameter "k" (look at wrappers.R, this
...
#
# The CV pruned tree has removed two of the predictors (S1 and S3)
# and has less leaf nodes (8 or 9, varies from run to run due to
# randomness in the CV process).
...
# at most only a little worse than the full tree, but can do a lot better
...
#
# So in this case, our random forest has an (approximate) R^2 of about 0.44 (43.99/100)
...
# of being a model you can understand. Obviously no one can make sense of 500 trees while
...
# a 98% increase (doubling) of our mean-squared error.
#
# Using this, it seems that BMI, BP and S5 are the stand-out important variables as omitting
# them would lead to large increases in prediction error.
#
